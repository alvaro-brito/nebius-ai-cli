...
import { NebiusAgent, ChatEntry } from "../../agent/nebius-agent";
...
function ChatInterfaceWithAgent({ agent }: { agent: NebiusAgent }) {
  ...
  useEffect(() => {
    ...
    const logoOutput = cfonts.render("NEBIUS CLI", {
      font: "block",
      align: "left",
      colors: ["green"],
      space: true,
    ...
    const logoString = (logoOutput as any).string || logoOutput.toString();
    if (logoString) {
      ...
      {chatHistory.length === 0 && !confirmationOptions && (
        <Box flexDirection="column" marginBottom={2}>
          ...
          <Box marginTop={1} flexDirection="column">
            ...
            <Text color="gray">
              3. Create NEBIUS.md files to customize your interactions with Nebius.
            </Text>
            <Text color="gray">
...
export default function ChatInterface({ agent }: ChatInterfaceProps) {
  const [currentAgent, setCurrentAgent] = useState<NebiusAgent | null>(
    agent || null
  );

  const handleApiKeySet = (newAgent: NebiusAgent) => {
