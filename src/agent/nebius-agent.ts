import { NebiusClient, NebiusMessage, NebiusToolCall, NebiusModelType, NEBIUS_MODELS } from "../nebius/nebius-client";
import {
  NEBIUS_TOOLS,
  addMCPToolsToNebiusTools,
  getAllNebiusTools,
  getMCPManager,
...
export interface ChatEntry {
  ...
  content: string;
  timestamp: Date;
  toolCalls?: NebiusToolCall[];
  toolCall?: NebiusToolCall;
...
export class NebiusAgent extends EventEmitter {
  ...
  private messages: NebiusMessage[] = [];
  ...
  constructor(apiKey: string, baseURL?: string, model?: string) {
    ...
    const modelToUse = (model || savedModel || NEBIUS_MODELS.QWEN) as NebiusModelType;
    this.nebiusClient = new NebiusClient(apiKey, modelToUse, baseURL);
    ...
    this.nebiusClient.setLogCallback((message: string) => {
    ...
    this.messages.push({
      role: "system",
      content: `You are Nebius AI CLI, an AI assistant powered by Nebius that helps with file editing, coding tasks, and system operations.${customInstructionsSection}
...
You have access to various tools for file operations, bash commands, and system tasks. Always be helpful and accurate in your responses.`
  ...
  private isNebiusModel(): boolean {
    const currentModel = this.nebiusClient.getCurrentModel();
    return Object.values(NEBIUS_MODELS).includes(currentModel as NebiusModelType);
  }
  ...
  async processUserMessage(message: string): Promise<ChatEntry[]> {
    ...
    try {
      const tools = await getAllNebiusTools();
      let currentResponse = await this.nebiusClient.chat(
        this.messages,
        tools,
        undefined
      );
      ...
      while (toolRounds < maxToolRounds) {
        ...
        if (!assistantMessage) {
          throw new Error("No response from Nebius");
        }
        ...
        if (
          ...
          currentResponse = await this.nebiusClient.chat(
            this.messages,
            tools,
            undefined
          );
  ...
  async *processUserMessageStream(
    ...
    try {
      ...
      while (toolRounds < maxToolRounds) {
        ...
        const stream = this.nebiusClient.chatStream(
          this.messages,
          tools,
          undefined
        );
  ...
  private async executeTool(toolCall: NebiusToolCall): Promise<ToolResult> {
    try {
  ...
  private async executeMCPTool(toolCall: NebiusToolCall): Promise<ToolResult> {
    try {
  ...
  getCurrentModel(): string {
    return this.nebiusClient.getCurrentModel();
  }

  setModel(model: string): void {
    this.nebiusClient.setModel(model);
  ...
  private async summarizeContext(): Promise<void> {
    ...
    try {
      ...
      const summaryMessage: NebiusMessage = {
        role: 'assistant',
  ...
  getLastErrorDetails() {
    return this.nebiusClient.getLastErrorDetails();
  }

  clearLastErrorDetails(): void {
    this.nebiusClient.clearLastErrorDetails();
  }
}
