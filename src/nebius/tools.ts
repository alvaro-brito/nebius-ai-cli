import { NebiusTool } from "./nebius-client";
import { MCPManager, MCPTool } from "../mcp/client";
import { loadMCPConfig } from "../mcp/config";

export const NEBIUS_TOOLS: NebiusTool[] = [
  {
    type: "function",
    function: {
      name: "view_file",
...
export function convertMCPToolToNebiusTool(mcpTool: MCPTool): NebiusTool {
  return {
    type: "function",
    function: {
      name: mcpTool.name,
...
export function addMCPToolsToNebiusTools(baseTools: NebiusTool[]): NebiusTool[] {
  if (!mcpManager) {
    return baseTools;
  }
  
  const mcpTools = mcpManager.getTools();
  const nebiusMCPTools = mcpTools.map(convertMCPToolToNebiusTool);
  
  return [...baseTools, ...nebiusMCPTools];
}

export async function getAllNebiusTools(): Promise<NebiusTool[]> {
  const manager = getMCPManager();
  await manager.ensureServersInitialized();
  return addMCPToolsToNebiusTools(NEBIUS_TOOLS);
}
