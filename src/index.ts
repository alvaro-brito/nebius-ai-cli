...
import { NebiusAgent } from "./agent/nebius-agent";
import { NEBIUS_MODELS } from "./nebius/nebius-client";
...
async function saveCommandLineSettings(apiKey?: string, baseURL?: string): Promise<void> {
  try {
    ...
    if (apiKey) {
      ...
      console.log("✅ API key saved to ~/.nebius/user-settings.json");
    }
    if (baseURL) {
      manager.updateUserSetting('baseURL', baseURL);
      console.log("✅ Base URL saved to ~/.nebius/user-settings.json");
    }
  } catch (error) {
...
async function handleCommitAndPushHeadless(
  ...
  try {
    const agent = new NebiusAgent(apiKey, baseURL, model);
...
program
  .name("nebius")
  .description(
    "A conversational AI CLI tool powered by Nebius with text editor capabilities"
  )
  .version("1.0.1")
  .option("-d, --directory <dir>", "set working directory", process.cwd())
  .option("-k, --api-key <key>", "Nebius API key (or set NEBIUS_API_KEY env var)")
  .option(
    "-u, --base-url <url>",
    "Nebius API base URL (or set NEBIUS_BASE_URL env var)"
  )
  .option(
    "-m, --model <model>",
    "AI model to use (google/gemma-3-27b-it, Qwen/QwQ-32B) (or set NEBIUS_MODEL env var)"
  )
  .option(
    "-p, --prompt <prompt>",
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (!apiKey) {
        console.error(
          "❌ Error: API key required. Set NEBIUS_API_KEY environment variable, use --api-key flag, or save to ~/.nebius/user-settings.json"
        );
        process.exit(1);
      }
      ...
      const agent = new NebiusAgent(apiKey, baseURL, model);
      console.log("🤖 Starting Nebius AI CLI Conversational Assistant...\n");

      ensureUserSettingsDirectory();

      render(React.createElement(ChatInterface, { agent }));
    } catch (error: any) {
      console.error("❌ Error initializing Nebius AI CLI:", error.message);
      process.exit(1);
    }
  });
...
program
  ...
  .action(() => {
    console.log("📋 Available Nebius AI models:\n");
    Object.entries(NEBIUS_MODELS).forEach(([key, value]) => {
      console.log(`  • ${value} (${key})`);
    });
    console.log("\nUse the --model flag to specify a model:");
    console.log("  nebius --model Qwen/Qwen3-235B-A22B-Instruct-2507 (default)");
    console.log("  nebius --model deepseek-ai/DeepSeek-R1-0528");
    console.log("  nebius --model meta-llama/Llama-3.3-70B-Instruct");
  });
