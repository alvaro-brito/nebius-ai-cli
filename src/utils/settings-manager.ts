...
/**
 * User-level settings stored in ~/.nebius/user-settings.json
 * These are global settings that apply across all projects
 */
export interface UserSettings {
  apiKey?: string;           // Nebius API key
...
/**
 * Project-level settings stored in .nebius/settings.json
...
export class SettingsManager {
  ...
  private constructor() {
    // User settings path: ~/.nebius/user-settings.json
    this.userSettingsPath = path.join(os.homedir(), '.nebius', 'user-settings.json');
    
    // Project settings path: .nebius/settings.json (in current working directory)
    this.projectSettingsPath = path.join(process.cwd(), '.nebius', 'settings.json');
  }
  ...
  /**
   * Load user settings from ~/.nebius/user-settings.json
   */
  ...
  /**
   * Save user settings to ~/.nebius/user-settings.json
   */
  ...
  /**
   * Load project settings from .nebius/settings.json
   */
  ...
  /**
   * Save project settings to .nebius/settings.json
   */
  ...
  public getCurrentModel(): string {
    ...
    return DEFAULT_PROJECT_SETTINGS.model || 'Qwen/Qwen3-235B-A22B-Instruct-2507';
  }
  ...
  public getApiKey(): string | undefined {
    ...
    const envApiKey = process.env.NEBIUS_API_KEY;
    if (envApiKey) {
      return envApiKey;
    }
  ...
  public getBaseURL(): string {
    // First check environment variable
    const envBaseURL = process.env.NEBIUS_BASE_URL;
    if (envBaseURL) {
      return envBaseURL;
    }
